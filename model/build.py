...
class Swin4MRI(L.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Compute metrics
        score = f1_score(y.cpu().numpy(), predicted_classes.detach().cpu().numpy(), average='macro')
        acc = accuracy_score(y.cpu().numpy(), predicted_classes.detach().cpu().numpy())
        self.log_dict({'test_loss': loss, "test_accuracy": acc, "test_f1_score": score}, on_step=True)

        # Confusion matrix
        cm = confusion_matrix(y.cpu().numpy(), predicted_classes.detach().cpu().numpy(),
                              labels=[0, 1, 2, 3])

        self.test_steps_outputs.append(cm)

    def on_test_end(self):
        tensorboard = self.logger.experiment

        cm = compute_confusion_matrix(self.test_steps_outputs, labels=[0, 1, 2, 3])
        self.test_steps_outputs.clear()
        tensorboard.add_image("Confusion_matrix", cm)
